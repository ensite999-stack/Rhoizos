...
function requiredCost(value:unknown){
  ...
  if(!Number.isFinite(n)||n<=0)throw new Error("Registrar cost must be greater than zero.");
  return n;
}
  ...
  try{
    ...
    await db()`
      update tld_prices set
        ...
        override_register=null,override_renew=null,override_transfer=null,
        featured=${featured},active=${active},updated_at=now()
      where tld=${tld}
    `;
    await audit(admin.id,"pricing.tld.update","tld_price",tld,{
      costRegister,costRenew,costTransfer,featured,active
    });
