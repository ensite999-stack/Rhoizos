...
export default function Dropcatch(){
  ...
  async function load(){
    ...
    setAuthRequired(false);setItems(data.items||[]);setWindowOpen(Boolean(data.windowOpen));
  }
  useEffect(()=>{load();},[]);
  ...
  async function create(event:FormEvent<HTMLFormElement>){
    ...
    const response=await fetch("/api/dropcatch",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      domain:form.get("domain"),years:Number(form.get("years")||1),
      private:form.get("private")==="on",autoRenew:form.get("autoRenew")==="on"
    })});
    if(response.status===401){setAuthRequired(true);setBusy(false);return;}
  ...
  async function cancel(id:string){
    ...
    if(!response.ok){setError(data.error||t("dropcatch.cancelFailed"));return;}await load();
  }
  async function checkout(item:RequestItem){
    ...
    const data=await response.json();
    if(!response.ok){setError(data.error||t("dropcatch.checkoutFailed"));return;}location.href=data.checkoutUrl;
  }
  ...
  return <div className="page wide serviceLanding">
    ...
    {authRequired?<div className="serviceGate">
      <strong>{t("dropcatch.signInTitle")}</strong>
      <p>{t("dropcatch.signInCopy")}</p>
      <div className="serviceCta">
        <Link className="primaryLink" href="/login?next=%2Fdropcatch">{t("login.button")}</Link>
        <Link className="secondaryLink" href="/signup?next=%2Fdropcatch">{t("login.create")}</Link>
      </div>
    </div>:<>
      <div className="warningCard">
        <strong>{loading?t("common.loading"):(windowOpen?t("dropcatch.windowOpen"):t("dropcatch.windowClosed"))}</strong>
      ...
      <div className="featureList">
        {items.map(item=><div className="featureListRow dropcatchRow" key={item.id}>
          <div><strong>{item.domain}</strong><span>{t("dropcatch.status")}: {item.status.replace(/_/g," ")} · {t("dropcatch.attempts")}: {item.attempts}</span>
          ...
          <div className="actions">
            {(item.status==="caught"||item.status==="awaiting_payment")&&item.payment_status!=="paid"&&<button className="primary" onClick={()=>checkout(item)}>{t("dropcatch.payToClaim")}</button>}
